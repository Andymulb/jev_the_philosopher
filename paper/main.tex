...
\title{A Quarter of a Second to Decide Who Lives:\\
  A Decision Model Takes the Bunker Exam from the Film \emph{The Philosophers}}
\author{Andreas~Blum%
...
\begin{abstract}
\noindent In the film \emph{The Philosophers} (2013, also released as \emph{After the Dark}), a teacher asks his class which 10 of 21 people should enter a bunker to survive a nuclear apocalypse~\cite{huddles2013}. We put \NQuestionIds{} questions from the film to Jev~1.13, a \enquote{decision model} that does not write text but returns a typed choice with probabilities~\cite{typesafe_blog}. Every question was asked \NRuns{} times (\NRequests{} requests in total). Jev gave the same answer as the film's characters on \NAgree{} of \NFilmQuestions{} questions, a different answer on \NDisagree{}, and was undecided on \NUndecided{}. Its answers barely changed between runs, but they did change when the same dilemma was worded differently. Jev needed a median of \LatMedianSec{}\,s per answer, whether the question was easy or hard, while people in a classic laboratory study needed \HumanFastest--\HumanSlowest\,s per moral dilemma~\cite{greene2001}.
\end{abstract}
...
We asked three simple questions: what does Jev decide, and how often does it decide like the characters in the film? How consistent is it, across repeated runs and across different wordings of the same dilemma? And how long does it take, compared with people?

This paper makes four contributions:
\begin{enumerate}[label=(\arabic*),leftmargin=1.6em]
  \item a catalogue of \NQuestionIds{} moral decisions from the film, written as typed questions together with the answers the film's characters give, published as data (Section~\ref{sec:method}, Appendix~\ref{app:results});
  \item a measurement of one decision model's answers to them, repeated \NRuns{} times per question, covering stability, agreement with the film and sensitivity to wording (Sections~\ref{sec:stability}--\ref{sec:framing});
  \item a latency measurement showing that the model's decision time is independent of the difficulty of the question, set against published human decision times (Section~\ref{sec:timing});
  \item a discussion of what decisions at this speed and price mean for the fields that are adopting them (Section~\ref{sec:implications}).
\end{enumerate}
...
\noindent The film frames the rounds as a contest between cold calculation and human values~\cite{blimey_explained,lovepirate2014}. Critics have called its philosophy shallow~\cite{philosophyinfilm2018,horizonfringe2014,neverfeltbetter2014}, but its questions are real and easy to state, which is exactly what we need.
...
wording of the question carries part of the answer.
...
Put through the bunker exam from \emph{The Philosophers}, the decision model Jev answers every
dilemma in about a quarter of a second, gives nearly identical answers when asked again, and
agrees with the film's characters on roughly a third of the questions with a known answer. Its
answers depend on how a dilemma is worded, and its speed does not depend on how hard the
dilemma is. People take \SpeedupLow{} to \SpeedupHigh{} times longer and slow down precisely
when a decision goes against their instinct.
