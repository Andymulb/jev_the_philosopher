...
\begin{enumerate}[label=(\arabic*),leftmargin=1.6em]
  ...
  \item measurements of how stable those answers are across repetitions, how far they move when a
    dilemma is reworded, and how far they move when the film supplies one further fact about each
    candidate --- without the thinner answers having looked any less decided
    (Sections~\ref{sec:stability}--\ref{sec:framing});
...
The step from round one to round two is also a natural experiment on context: the same 21 people
are judged again, with one further fact attached to each. Jev used it. Its yes-probability for a
person moved by a median of \ContextMedianShift{} and by as much as \ContextMaxShift{}---the estate
agent who turns out to be a trained midwife rises from 0.18 to 0.85---\ContextMovers{} of the 21
moved by more than 0.20, and \ContextPlacesChanged{} of the ten places changed hands. The film's
class revised its votes in the same direction, dropping the surgeon over the Ebola risk and the
electrician over his illness. What did \emph{not} change is how decided the model sounded. In round
one, knowing nothing but a profession, its answers lay a median of \RoundOneDistHalf{} from 0.5 and
\RoundOneInBand{} of the 21 fell inside the undecided band; in round two, with a second fact per
person, the figures were \RoundTwoDistHalf{} and \RoundTwoInBand. The extra context changed the
answers substantially without the earlier, thinner answers having looked any less certain.
...
We did not vary context deliberately---rounds two and three change the rules as well as the
